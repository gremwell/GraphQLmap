...
def requester(url, method, payload, headers=None, use_json=False, proxies=None):
    if method == "POST" or use_json:
        ...
        r = requests.post(url, data=new_data, verify=False, headers=new_headers, proxies=proxies)
        if r.status_code == 500:
            print("\033[91m/!\ API didn't respond correctly to a POST method !\033[0m")
            return None
    else:
        r = requests.get(url + "?query={}".format(payload), verify=False, headers=headers, proxies=proxies)
    return r
...
def parse_args():
    ...
    parser.add_argument('--proxy', action ='store', dest='proxy', help="Optional HTTP proxy", nargs='?',  const=True, default="")
